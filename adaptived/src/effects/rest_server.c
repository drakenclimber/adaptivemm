...
const char * const hello_world = "<html><body>Hello World</body></html>";
		     ...
		     const struct adaptived_cause * const cse)
{
	...
	opts->endpoint = malloc(sizeof(char) * strlen(endpoint_str) + 1);
	if (!opts->endpoint) {
		ret = -ENOMEM;
		goto error;
	}

	strcpy(opts->endpoint, endpoint_str);
	opts->endpoint[strlen(endpoint_str)] = '\0';
