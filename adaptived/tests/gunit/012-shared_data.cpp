...
TEST_F(SharedDataTest, InvalidShare)
{
	...
	ret = adaptived_write_shared_data(NULL, ADAPTIVED_SDATA_STR, data, 0);
	...
	ret = adaptived_write_shared_data(&cse, (enum adaptived_sdata_type)-1, data, 0);
	ASSERT_EQ(ret, -EINVAL);

	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_CNT, data, 0);
	ASSERT_EQ(ret, -EINVAL);

	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_STR, NULL,
...
TEST_F(SharedDataTest, CustomDataShare)
{
	...
	custom_data = (int *)malloc(sizeof(int) * 4);
	...
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_CUSTOM, custom_data, flags);
	ASSERT_EQ(ret, 0);
	...
	read_data_ptr = (int *)read_data;

	ASSERT_EQ(read_data_ptr[0], 123);
	ASSERT_EQ(read_data_ptr[1], 456);
	ASSERT_EQ(read_data_ptr[2], 7890);
	ASSERT_EQ(read_data_ptr[3], 98765);
...
TEST_F(SharedDataTest, StringDataShare)
{
	...
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_STR, src,
					  ADAPTIVED_SDATAF_PERSIST);
	ASSERT_EQ(ret, 0);
...
TEST_F(SharedDataTest, CgroupFloatDataShare)
{
	...
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_CGROUP, src, 0);
	ASSERT_EQ(ret, 0);
...
TEST_F(SharedDataTest, MultipleDataShares)
{
	...
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_STR, src_str, 0);
	ASSERT_EQ(ret, 0);
	...
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_CGROUP, src_cgstr, 0);
	ASSERT_EQ(ret, 0);
