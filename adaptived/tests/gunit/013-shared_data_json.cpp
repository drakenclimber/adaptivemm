...
void build_nameval_ll(struct adaptived_name_and_value **nameval,
	const char * const name, long long value)
{
	struct adaptived_name_and_value *tmpc;
	struct adaptived_cgroup_value *tmpv;

	tmpc = (struct adaptived_name_and_value *)malloc(
		sizeof(struct adaptived_name_and_value));
	ASSERT_NE(tmpc, nullptr);

	tmpc->name = (char *)malloc(sizeof(char) * (strlen(name) + 1));
	ASSERT_NE(tmpc->name, nullptr);
	sprintf(tmpc->name, "%s", name);
...
void build_nameval_str(struct adaptived_name_and_value **nameval,
...
void build_nameval_float(struct adaptived_name_and_value **nameval,
...
{
	const char * const expected1 = "{ \"memory.max\": { \"database.slice\": 22446688, \"cloud.slice\": 113355779 }, \"cpuset.cpus.effective\": { \"database.slice\": \"1-3,7,9,13\" }, \"cpu.uclamp.min\": { \"database.slice\": 9.876500129699707 }, \"cpu.max\": { \"database.slice\": \"100000 max\", \"cloud.slice\": \"500000 1000000\" } }";
	const char * const expected2 = "{ \"memory.max\": { \"database.slice\": 22446688, \"cloud.slice\": 113355779 } }";
	...
	build_cgsetval_ll(&c1, "database.slice", "memory.max", 22446688);
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_CGROUP_SETTING_VALUE, c1, NULL, 0);
	ASSERT_EQ(ret, 0);
	...
	build_cgsetval_ll(&c5, "cloud.slice", "memory.max", 113355779);
...
TEST_F(SharedDataJsonTest, NameAndValueSettings)
{
	const char * const expected1 = "{ \"memory.max\": 12345678, \"cpuset.cpus.effective\": \"1-3,7,9,13\", \"cpu.uclamp.min\": 9.876500129699707, \"cpu.max\": \"100000 max\", \"memory.current\": 1133557799, \"cpu.weight\": \"4096\" }";
	const char * const expected2 = "{ \"memory.max\": 12345678 }";
	const char * const expected3 = "{ \"cpu.weight\": \"4096\" }";
	struct adaptived_name_and_value *c1, *c2, *c3, *c4, *c5, *c6;
	struct json_object *json_obj1 = NULL, *json_obj2 = NULL,
			   *json_obj3 = NULL;
	struct adaptived_cause cse;
	const char *json_str;
	...
	populate_cause(&cse, 3);

	build_nameval_ll(&c1, "memory.max", 12345678);
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_NAME_VALUE, c1, NULL, 0);
	ASSERT_EQ(ret, 0);

	build_nameval_str(&c2, "cpuset.cpus.effective", "1-3,7,9,13");
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_NAME_VALUE, c2, NULL, 0);
	ASSERT_EQ(ret, 0);

	build_nameval_float(&c3, "cpu.uclamp.min", 9.8765);
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_NAME_VALUE, c3, NULL, 0);
	ASSERT_EQ(ret, 0);

	build_nameval_str(&c4, "cpu.max", "100000 max");
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_NAME_VALUE, c4, NULL, 0);
	ASSERT_EQ(ret, 0);

	build_nameval_ll(&c5, "memory.current", 1133557799);
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_NAME_VALUE, c5, NULL, 0);
	ASSERT_EQ(ret, 0);

	build_nameval_str(&c6, "cpu.weight", "4096");
	ret = adaptived_write_shared_data(&cse, ADAPTIVED_SDATA_NAME_VALUE, c6, NULL, 0);
	ASSERT_EQ(ret, 0);
	...
	ret = adaptived_sdata_to_json(&cse, "cpu.weight", &json_obj3);
	ASSERT_EQ(ret, 0);

	json_str = json_object_to_json_string(json_obj3);
	ASSERT_STREQ(json_str, expected3);
